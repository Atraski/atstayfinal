  ...
  const inc = () => {
    if (num < Increment) {
     ...
     const inc1=()=>{
      ...
      const inc2 = () => {
    if(numss <num){
      const s2 = parseInt(numss) + 1;

      setnumss(s2);
    }
    else{
      alert("No More Children are allowed")
    }
...
<input type="date" placeholder="asdf" value={checkin} min={currentDate} onChange={(e)=>{setcheckin(e.target.value)}} style={{width:'100%' , padding:'10px',border:"none"}}/></div>
<div   style={{ backgroundColor:"white",height:"100%",width:"50%",border:"0.1px solid #66cccc" ,}}><p className='mt-4 ms-2'>Check Out</p>
<input type="date" placeholder="asdf"  value={checkout} min={checkin || currentDate} onChange={(e)=>{setcheckout(e.target.value)}} style={{width:'100%' , padding:'10px',border:"none"}}/></div>
...
<div style={{ backgroundColor:"white", height:"100px",display:"flex",justifyContent:"center",alignItems:'center',border:"0.1px solid #66cccc"}}> 
 <button className='' style={{ backgroundColor:"#66cccc" , textTransform:'uppercase', border:'none' , borderRadius:'25px' , padding:'5px 15px'}} ><h2 style={{fontSize:'20px', marginTop:'5px'}} onClick={()=>{
  ...
  if(!checkin || !checkout){
    ...
    // Navigate to the next page (add your navigation logic here)
        ...
        <div className="whitebox w-100 bg-white d-none container1" style={{height:'80px',justifyContent:'space-between',alignItems:'center',zIndex:20 , position:"fixed" , bottom:'0%'}}>
